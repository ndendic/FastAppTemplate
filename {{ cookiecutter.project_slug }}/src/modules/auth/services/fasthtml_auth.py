...
class FastHTMLAuth:
    ...
    async def login(self, request, email, password):
        try:
            ...
            logfire.error(f"FastHTML login error: {e}")
            return None
    ...
    async def register(self, request, password, email):
        try:
            ...
            is_first_user = len(User.query()) == 0
            
            user = User()
            user.password = hashed_password
            user.email = email
            user.is_admin = is_first_user  # Set admin flag if first user
            # Also set role to admin for first user
            if is_first_user:
                user.role = "admin" 
            else:
            ...
            logfire.error(f"FastHTML registration error: {e}")
            return None
    ...
    async def request_password_reset(self, request, email):
        try:
            ...
            logfire.error(f"FastHTML password reset request error: {e}")
            return False
    ...
    async def validate_otp(self, request, email, otp):
        try:
            ...
            logfire.error(f"FastHTML OTP validation error: {e}")
            return False
    ...
    async def reset_password(self, request, token, new_password):
        try:
            ...
            logfire.error(f"FastHTML password reset error: {e}")
            return False
    ...
    async def _send_otp_email(self, request, user_email, otp):
        ...
        try:
            ...
            logfire.info(f"Password reset OTP sent to {user_email}")
            return True
        except asyncio.TimeoutError:
            add_custom_toast(request.session, "Email sending timed out", "error")
            logfire.error("Email sending timed out")
            return False
        except Exception as e:
            add_custom_toast(
                request.session, f"Error sending password reset OTP: {e}", "error"
            )
            logfire.error(f"Error sending password reset OTP: {e}")
            return False
