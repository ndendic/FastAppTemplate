...
toast_css = """
.fh-toast-container {
    position: fixed; bottom: 20px; left: 10%; transform: translateX(-50%); z-index: 1000;
...
def toast(typ: str, msg: str) -> Div:
    return Div(
        ...
        id=f"toast-{typ}",
        role="alert",
        cls=f"fh-toast-{typ} flex items-center w-full max-w-xs p-4 mb-4 rounded-lg shadow",
    )

def MUIToast(typ: str, msg: str) -> Toast:
    return Toast(msg, alert_cls=f"alert-{typ}", id=f"toast-{typ}")
        

def render_toasts(sess):
    # toasts = [MUIToast(typ, msg) for msg, typ in sess.pop(sk, [])]
